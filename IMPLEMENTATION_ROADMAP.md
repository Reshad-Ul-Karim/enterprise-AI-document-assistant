...
| Corpus tokens (full) | **122,119** = **46.6%** of the window | tekken v13 — **never chars/4, never tiktoken, never `MistralTokenizer.v3()`** |
| Corpus tokens (indexed scope) | **90,462** | same |
| Handbook tokens (pinned) | **3,081** | same |
| `mistral-large-2512` context | **262,144** | Mistral model card + verified against `GET /v1/models` with a live key |
| Sections detected | **342** | dual-grammar + LIS, idx 33–156 |
| Chunks | **482** (472 statute + 10 handbook) | sub-split >2000 chars |
| Index size | **0.74 MB** | 482 × 384 float32 |
| Exact cosine + top-8 | **0.0076 ms** | mean of 2000, **real** embedded chunks |
| Peak RSS (deployed) | **~435 MB** | vs Render free's **512 MB** ceiling — onnxruntime is ~280 MB of it |

> **These are generated by `python -m src.ingest.corpus_stats`, from the system that is actually built.**
> An earlier draft of this table carried the *council's prototype* numbers — 122,204 tokens, 399 chunks, 0.596 MB — while the
> README carried the built system's. **Two documents in one repo, disagreeing about the same fact.** That is the exact failure this
> rule exists to prevent, committed in the document that preaches the rule. Fixed by generating it. **If you edit a number here by
> hand, you have reintroduced the bug.**

**Delete on sight — these are poisoned numbers from superseded drafts:**
`113,240` · `134,631` · `128k window` · `43%` · `481 vectors` · `399 chunks` · `0.596 MB` · `0.023 ms` · `35.1 ms` · `333 of 335` ·
...
|---|---|---|
| **RAG** | **KEEP**, reframed | Corpus is 122,119 tok / 262,144 window = **46.6%. It fits.** RAG is a *defended choice*, and the oracle proves it. |
...
| **Retrieval** | Hybrid BM25 + dense, **RRF k=60**, top-8, whole-section expansion | Asymmetric: **pin the handbook (3,081 tok), retrieve only the statute**. |
